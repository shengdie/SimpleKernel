...
BUILD_NOW()
{
	...
	if [ ! -f "$KERNELDIR"/.config ]; then
		...
		elif [ "$BUIDL_L01F" -eq "1"]; then
			cp arch/arm/configs/l01fs_defconfig .config
		fi;
	fi;
	...
	#B Check that RAMDISK is for this Branch. and check if need to push changes before switch to needed branch.
	...
	cp -a ../simple-ramdisk-l01f/* ../ramdisk-tmp/
	...
	if [ -e "$KERNELDIR"/arch/arm/boot/zImage ]; then
		...
		../arm-eabi-4.7/bin/arm-eabi-strip --strip-unneeded ./READY-KERNEL/system/lib/modules/* 2>/dev/null
		...
		zip -r Kernel-Simple-KK-"$(date +"[%H-%M]-[%d-%m]-LG-L01F-V6")".zip * >/dev/null
		stat boot.img
		rm -f ./*.img
		cd ..
	else
...
select CHOICE in D800 D801 D802 D803 F320 LS980 VS980 L01F ALL; do
	case "$CHOICE" in
		...
		"VS980")
			...
			break;;
		"L01F")
			export KERNEL_CONFIG=l01fs_defconfig
			KERNEL_CONFIG_FILE=l01fs_defconfig
			BUILD_L01F=1;
			BUILD_NOW;
			break;;
		"ALL")
